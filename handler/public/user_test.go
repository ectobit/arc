...
func TestPasswordFromJSON(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"ok": {
			...
			&public.Password{Password: "h+z67{GxLSL~]Cl(I88AqV7w"}, "",
		},
	}
	...
	for n, test := range tests { //nolint:paralleltest
		...
		t.Run(n, func(t *testing.T) {
			...
			got, gotErr := public.PasswordFromJSON(buf, log)
			if test.wantErr != "" {
				if gotErr == nil {
...
func TestResetPasswordFromJSON(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"ok": {
			...
			&public.ResetPassword{PasswordResetToken: "test", Password: "h+z67{GxLSL~]Cl(I88AqV7w"}, "", //nolint:exhaustivestruct,lll
		},
	}
	...
	for n, test := range tests { //nolint:paralleltest
		...
		t.Run(n, func(t *testing.T) {
			...
			got, gotErr := public.ResetPasswordFromJSON(buf, log)
			if test.wantErr != "" {
				if gotErr == nil {
